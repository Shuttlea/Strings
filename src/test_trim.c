...
START_TEST(trim_2) {
  char s1[] =
      "It was the first time he had ever seen someone cook dinner on an "
      "elephant";
  char s2[] = "It was";
  char *s3 = s21_trim(s1, s2);
  ck_assert_pstr_eq(
      s3, "he first time he had ever seen someone cook dinner on an elephan");
  if (s3) free(s3);
}
END_TEST
...
START_TEST(trim_3) {
  char s1[] =
      "She learned that water bottles are no longer just to hold liquid";
...
START_TEST(trim_8) {
  ...
  ck_assert_pstr_eq(s3,
                    "Everyone says they love nature until they realize how "
...
Suite *test_trim() {
  ...
  TCase *test_group = tcase_create("s21_trim_test_group");
  ...
  tcase_add_test(test_group, trim_12);
  tcase_add_test(test_group, trim_13);
  tcase_add_test(test_group, trim_14);
  tcase_add_test(test_group, trim_15);
  tcase_add_test(test_group, trim_16);
