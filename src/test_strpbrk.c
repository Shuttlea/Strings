...
START_TEST(strpbrk_9) {
  ...
  char s2[] =
      "She couldnt understand why nobody else could see that the sky is full "
      "of cotton candy.";
  ck_assert_pstr_eq(strpbrk(s1, s2), s21_strpbrk(s1, s2));
}
END_TEST
...
START_TEST(strpbrk_11) {
  ...
  char s2[] =
      "He\t\n\b\0 found his art never progressed when he literally used his "
      "sweat and tears.";
  ck_assert_pstr_eq(strpbrk(s1, s2), s21_strpbrk(s1, s2));
}
END_TEST
...
Suite *test_strpbrk() {
  ...
  TCase *test_group = tcase_create("s21_strpbrk_test_group");
