...
void *s21_memset(void *str, int c, size_t n) {
  ...
  if (length >= n)
    ...
    fprintf(stderr,
            "Error: writing %d bytes into a region of size %d overflows the "
            "destination\n",
            (int)n, (int)length + 1);
    exit(1);
  }
...
char *s21_strerror(int errnum) {
  ...
  if (errnum >= 0 && errnum < ERRORS_COUNT)
    ...
    s21_sprintf(buf, UNKNOWN_ERROR, errnum);
    error = buf;
  }
...
void *s21_insert(const char *src, const char *str, size_t start_index) {
  ...
  if (src && str && start_index <= s21_strlen(src)) {
    res = malloc(sizeof(char) * (s21_strlen(src) + s21_strlen(str)) +
                 sizeof(char));
...
void *s21_trim(const char *src, const char *trim_chars) {
  ...
  if (src && trim_chars) {
    ...
    int start_index = 0, end_index = srclen - 1, equal = 1;
    for (s21_size_t i = 0; i < srclen && equal; i++) {
      equal = 0;
      for (s21_size_t j = 0; j < trimlen; j++)
        if (src[i] == trim_chars[j]) equal = 1;
      if (!equal) start_index = i;
    }
