...
START_TEST(strchr_3) {
  char s[] =
      "When he encountered maize for the first time, he thought it incredibly "
      "corny.";
  int ch = ' ';
  ck_assert_pstr_eq(strchr(s, ch), s21_strchr(s, ch));
}
...
START_TEST(strchr_6) {
  char s[] =
      "Blue sounded too \tcold at\t the time and yet it\n seemed to work for\0 "
      "gin.";
  int ch = '\t';
  ck_assert_pstr_eq(strchr(s, ch), s21_strchr(s, ch));
}
...
START_TEST(strchr_8) {
  char s[] =
      "She had \tthat \0tint of craziness in her soul that made \0her believe "
...
START_TEST(strchr_9) {
  char s[] =
      "Truth in advertising and dinosaurs with skateboards have much in "
      "common.";
  int ch = 'a';
  ck_assert_pstr_eq(strchr(s, ch), s21_strchr(s, ch));
}
...
Suite *test_strchr() {
  ...
  TCase *test_group = tcase_create("s21_strchr_test_group");
